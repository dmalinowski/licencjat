...
\begin{thh2}[Jordana -- H\"oldera  \textcolor{red}{ -- odsyłacz}] $ $ \\
  Jeżeli $G$ jest grupą skończoną, to istnieje w niej skończony ciąg podgrup
    $$ G = G_n > G_{n-1} > \cdots > G_1 > G_0 = \{e\} $$
  taki, że $G_{i-1}$ jest podgrupą normalną $G_{i}$ oraz $G_{i}/G_{i-1}$ jest grupą prostą dla $i = 1, \cdots, n$.

  Ponadto dla każdego innego ciągu podgrup
    $$ G = H_m > H_{m-1} > \cdots > H_1 > H_0 = \{e\} $$
  spełniającego powyższe warunki zachodzi $n = m$ oraz $G_{i}/G_{i-1} \simeq H_{\pi(i)}/H_{\pi(i)-1}$ 
  dla $i = 1, \cdots, n$ oraz pewnej permutacji indeksów $\pi \in S_{n}$.
\end{thh2}
...
% podać źródło treści twierdzenia?
...
Kolejny rozdział, \emph{Prostota grupy alternującej $A_n$}, zaczyna się od przypomnienia definicji
grupy symetrycznej, cyklów oraz grupy alternującej.
...
Udowodnimy w nim związki między tymi pojęciami, maksymalność grup izotropii przy działaniu prymitywnym,
...
%TODO
%napisać coś o składaniu permutacji, tzn. że inaczej niż funkcji, lub przepisać na takie samo
...
\begin{proof}
    ...
    Jeżeli $\sigma$ zawiera cykl długości $k \ge 3$, to permutacji o typie $\tau$, których rozkład na cykle rozłączne zawiera cykl $(1, a_1, a_2, \cdots, a_k)$
...
\begin{proof}
  ...
  $$
  Natomiast, gdy $n \ge 3$ oraz $j \ne k$ to z przechodniości naturalnego działania grupy $A_n$ na zbiorze $\{1, 2, \cdots, n \}$ 
  otrzymujemy pewien element $\pi \in A_n$ taki, że $\pi(1) = k$. Niech $l = \pi^{-1}(j) \ne 1$ oraz $P_\pi$ -- macierz permutacji $\pi$,
...
ale jak wcześniej pokazaliśmy, działanie 2-przechodnie na biorze 3 elementowym jest też 3-przechodnie, stąd $\tilde{\rho}$ zadaje bijekcję między $PSL_2(\mathbb{F}_2)$
...
\begin{proof}[Dowód (II sposób)]
  ...
  Rzeczywiście, dla parami różnych $a, b, c, d, e \in \{ 1, 2, \cdots, n \}$ mamy
  $$ [(b, c, d)(a, c, e)] = (b, c, d)(a, c, e)(d, c, b)(e, c, a) = $$
  $$ = (b, e, a, c, d)(d, a, e, c, b) = (a, b, c)(d)(e) = (a, b, c)$$
  Zatem z lematu \ref{lemma_3cycles} grupa $A_n$ jest prosta.

  Rozważmy działanie $\rho$ grupy $A_n$ na trzy elementowych podzbiorach zbioru $\{ 1, 2, \cdots, n \}$,
  w~którym po prostu przestawiam elementy zbioru $\{1, 2, \cdots, n\}$, 
  ...
  która osobno permutuje zbiór $\{1, 2, 3\}$ i~osobno $\{4, 5, \cdots, n\}$.
  Zatem $A =  \langle (1, 2, 3)  \rangle \le H$ jest oczywiście przemienną grupą,
  ale jest również podgrupą normalną $H$, gdyż $(1, 2, 3)$ jest przemienne z permutacjami $\{4, 5, \cdots, n\}$
