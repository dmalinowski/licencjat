...
\begin{proof}
	...
	Niech $H = G_{x_0}$.
	Dostajemy, że $K \nleq H$, gdyż $k_0 \not \in H$, stąd również $H \lneq H K$.% coś dopisać coś o HK?

	Z lematu \ref{max_group} otrzymujemy, że $H$ jest podgrupą maksymalną w $G$.
	$H \lneq H K$, więc $H K = G$.
	Stąd każdy element $g \in G$ jest postaci $g = h k$, gdzie $h \in H$ oraz $k \in K$.

	Skoro działanie $\rho$ jest prymitywne, a więc tranzytywne, to z twierdzenia ...%TODO 
	$G_x$ jest sprzężone z $H$.
	...
	Sprzężenia $B$ są postaci $g^{-1} B g = k^{-1} h^{-1} B h k = k^{-1} B k \le B K$. % coś dopisać?
	...
	sprzeczność z założeniem, że $K$ jest właściwą podgrupą $G$, stąd $G$ jest grupą prostą.
\end{proof}
